    ...
    static public function buildRenderValue($key,$value)
    {
        ...
        switch ($valueMapType) {
            ...
            case "array":
                ...
                break;
            case "string":
                $html = "<ul style='margin-left: 14px; $ulStyle'>[$key] $arrow &ldquo;" . $value["__data"] . "&rdquo;</ul>";
                break;
            case "null":
    ...
    static protected function buildRender(array $varMap)
    {
        ...
        if (!is_array($varMap["__data"])) {
            if (is_string($varMap["__data"])) {
                return "<ul><span style='" . self::$summaryStyle . "'>&ldquo;" . $varMap['__data'] . "&rdquo;</span></ul>";
